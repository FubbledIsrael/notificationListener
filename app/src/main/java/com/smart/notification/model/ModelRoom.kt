...
class ModelRoom {
    ...
    suspend fun updateAd(ad: AdEntity) = withContext(Dispatchers.IO){
        try {
            adDao.updateById(ad.phone, ad.device, ad.city, ad.classification, ad.status, ad.id)
        } catch (e: Exception){
            println(e.message)
    ...
    fun getRecordByStatus(status: Int): LiveData<MutableList<RecordEntity>> = recordDao.getByStatus(status)

    suspend fun saveRecord(record: RecordEntity) = withContext(Dispatchers.IO){
        try {
            recordDao.add(record)
        } catch (e: Exception){
            recordDao.updateTimeAndAd(record.time, record.id_ad, record.phone)
            throw Exception(TypeError.UNKNOWN.name)
        }
    }
